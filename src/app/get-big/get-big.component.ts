...
import {filter, map, mergeMap, switchMap, switchMapTo, tap} from 'rxjs/operators';
import {DestinyClass, DestinyComponentType, MembershipType} from '../core/bungie.enums';
...
import {DestinyDefinition, DestinyInventoryBucketDefinition, DestinyInventoryItemDefinition} from '../core/manifest.model';
...
export class GetBigComponent implements OnInit {
  ...
  private items: DestinyDefinition<DestinyInventoryItemDefinition>;
  ...
  setMembership(membership: MembershipDisplayName): void {
    ...
    this.bungie.searchPlayer(membership.displayName).pipe(
      ...
      tap(rsp => {
        const allEquipment = this.getEquipmentAcrossAllCharacters(rsp);
        this.loadEquipmentByClass(allEquipment);
  ...
  private getEquipmentAcrossAllCharacters(profile: DestinyProfileResponse): Equipment[] {
    const inventories = [].concat(...Object.values(profile.characterInventories.data)
      .map(c => c.items.filter(i => i.itemInstanceId !== undefined)));
    const equipped = [].concat(...Object.values(profile.characterEquipment.data)
      .map(c => c.items.filter(i => i.itemInstanceId !== undefined)));

    return [].concat(inventories, equipped)
      .map(item => ({ item, instance: profile.itemComponents.instances.data[item.itemInstanceId] }));
  }

  private loadEquipmentByClass(equipment: Equipment[]): void {
    const weapons = equipment.filter(i => i.instance.damageType > 0);
    const warlockStuff$ = forkJoin(equipment
      .filter(i => i.instance.damageType === 0)
      .map(i => this.bungie.getDestinyEntityDefinition('DestinyInventoryItemDefinition', i.item.itemHash)
        .pipe(
          filter(d => d.classType === DestinyClass.Warlock),
          map(() => i)
        )));

    const hunterStuff$ = forkJoin(equipment
      .filter(i => i.instance.damageType === 0)
      .map(i => this.bungie.getDestinyEntityDefinition('DestinyInventoryItemDefinition', i.item.itemHash)
        .pipe(
          filter(d => d.classType === DestinyClass.Hunter),
          map(() => i)
        )));

    const titanStuff$ = forkJoin(equipment
      .filter(i => i.instance.damageType === 0)
      .map(i => this.bungie.getDestinyEntityDefinition('DestinyInventoryItemDefinition', i.item.itemHash)
        .pipe(
          filter(d => d.classType === DestinyClass.Titan),
          map(() => i)
        )));

    combineLatest(warlockStuff$, hunterStuff$, titanStuff$).subscribe(([w, h, t]) => {
      this.equipment = {
        warlock: this.getMostPowerfulEquipment(w.concat(weapons)),
        hunter: this.getMostPowerfulEquipment(h.concat(weapons)),
        titan: this.getMostPowerfulEquipment(t.concat(weapons))
      };
    });
  }

  private getMostPowerfulEquipment(equipment: Equipment[]): MostPowerfulEquipment {
    const mostPowerfulEquipment: MostPowerfulEquipment = {};
    mostPowerfulEquipment.kinetic = this.getMostPowerfulItemForBucket(equipment, BucketType.Kinetic);
    mostPowerfulEquipment.energy = this.getMostPowerfulItemForBucket(equipment, BucketType.Energy);
    mostPowerfulEquipment.power = this.getMostPowerfulItemForBucket(equipment, BucketType.Power);
    mostPowerfulEquipment.helmet = this.getMostPowerfulItemForBucket(equipment, BucketType.Helmet);
    mostPowerfulEquipment.arms = this.getMostPowerfulItemForBucket(equipment, BucketType.Arms);
    mostPowerfulEquipment.chest = this.getMostPowerfulItemForBucket(equipment, BucketType.Chest);
    mostPowerfulEquipment.legs = this.getMostPowerfulItemForBucket(equipment, BucketType.Legs);
    mostPowerfulEquipment.classItem = this.getMostPowerfulItemForBucket(equipment, BucketType.ClassItem);
    return mostPowerfulEquipment;
  }
  ...
  private getEquipment(characterId: string): Observable<Equipment[]> {
    return this.bungie.getCharacter(this.membershipId, characterId, {
      ...
      .pipe(
        map(c => {
          console.log(c, c.itemComponents);
